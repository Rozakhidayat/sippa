...
new class extends Component
{
    ...
    public function nextProgressStep($progresId)
    {
        $current = DevelopmentProgres::findOrFail($progresId);
        ...
        $nextProgres = DevelopmentProgres::where('submission_id', $this->submission->id)
            ->whereHas('development_task', function($query) use ($current) {
                $query->where('sort_order', '>', $current->development_task->sort_order);
            })
            ->join('development_tasks', 'development_progres.develop_task_id', '=', 'development_tasks.id')
            ->orderBy('development_tasks.sort_order', 'asc')
            ->select('development_progres.*')
            ->first();

        if ($nextProgres) {
            $nextProgres->update(['status' => 'proses']);
        } else {
            $this->submission->update(['status' => 'selesai']);
            $this->submission->user->notify(new \App\Notifications\SubmissionNotification(
            $this->submission, 
            "Aplikasi '{$this->submission->application_name}' telah selesai dikembangkan!"
        ));
        }
        
        $this->submission->load('develop_progres.development_task');
        session()->flash('success', 'Tahapan berhasil diperbarui!');
    }
};
?>
...
<div>
    <main id="main" class="main">
        <div class="pagetitle mb-3 d-flex justify-content-between align-items-center">
            <h1 class="mb-0">Proses Pengembangan</h1>
            <a href="/developer/pengajuan" wire:navigate class="btn btn-danger btn-sm">
            ...
            <div class="row g-4">
                    ...
                    <div class="card border-0 shadow-sm p-4 mt-4">
                        ...
                        <div class="mt-4">
                            ...
                            <div class="mb-3">
                                ...
                                <span class="badge bg-light text-dark border me-1">{{ $theme->item }}</span>
                                @endforeach
                            </div>
                            @empty
                            @endforelse
                ...
                <div class="col-lg-4">
                    <div class="card border-0 shadow-sm p-4 mb-4 text-start">
                        <div class="fw-semibold mb-4"><i class="bi bi-activity me-2 text-success"></i> Progres Saat ini</div>
                        <div class="mb-3">
                        ...
                        <div class="d-grid gap-2">
                            @php $activeTask = $submission->develop_progres->where('status', 'proses')->first(); @endphp
                            @if($activeTask)
                            <div class="text-center mt-4 pt-3 border-top">
                                <button wire:click="nextProgressStep({{ $activeTask->id }})"
                                    wire:loading.attr="disabled" class="btn btn-sm btn-success rounded-pill shadow-sm">
                                    <i class="bi bi-check2-circle me-1"></i>
                                    <span class="small">Selesaikan Tahap:</span>
                                    <strong class="small">
                                        {{ $activeTask->development_task?->task_name }}
                                    </strong>
                                </button>
